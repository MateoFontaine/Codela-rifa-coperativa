...
export default function UserRaffle() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        const res = await fetch('/api/random-available', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (data.results) {
          setRows(data.results as RaffleNumber[])
        }
      } catch (error) {
        console.error('Error cargando números aleatorios:', error)
      }
  ...
  const handleSearchGo = async (v: number) => {
    setSearchMode(true)
    setLoading(true)
    ...
    try {
      const { data, error } = await supa
        ...
        .gte('id', Math.max(1, v - 50))
        .lte('id', Math.min(TOTAL_NUMBERS, v + 50))
        .order('id', { ascending: true })

      if (!error && data) {
        setRows(data as RaffleNumber[])
        setSearchTarget(v)
      }
    } catch (error) {
  ...
  return (
    <div>
      {/* Tabs para mobile */}
      <div className="lg:hidden top-0 z-10 bg-white border-b mb-4">
        <div className="flex">
          <button
            ...
            <div className="bg-white border rounded-2xl p-4 shadow-sm">
              ...
              <div className="flex flex-wrap gap-3 text-xs">
                <div className="flex items-center gap-1.5">
                  <div className="w-5 h-5 rounded bg-emerald-100 border flex-shrink-0"></div>
                  <span className="text-gray-700">Disponible</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-5 h-5 rounded bg-amber-100 border flex-shrink-0"></div>
                  <span className="text-gray-700">Reservado</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-5 h-5 rounded bg-rose-100 border flex-shrink-0"></div>
                  <span className="text-gray-700">Vendido</span>
                </div>
              </div>
            </div>
